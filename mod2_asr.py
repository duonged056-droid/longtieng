...
def run_asr(audio_in: str, srt_out: str, batch_size: int = 4, mode="normal", video_in=None, roi=None, sim=0.85, fps=2.0):
    ...
    console.print(f"[bold blue]Đang bắt đầu nhận dạng âm thanh...[/bold blue]")
    
    # Bước 1: Load WhisperX Model
    console.print("▶ Đang nạp mô hình WhisperX (large-v3)...")
    compute_type = "int8_float16" if device == "cuda" else "int8"
    
    try:
        model = whisperx.load_model("large-v3", device, compute_type=compute_type, download_root=download_root)
        console.print("▶ Đang nhận dạng tiếng Trung (zh)...")
        ...
        # Bước 2: Alignment
        console.print("▶ Đang khớp thời gian (Alignment)...")
        ...
        console.print("▶ Đang kết xuất tệp phụ đề...")
